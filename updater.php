...
class WP_GitHub_Updater {
	...
	public function set_defaults() {
		...
		// error_log( var_export($plugin_data, true) );
		if ( ! isset( $this->config['plugin_name'] ) )
			$this->config['plugin_name'] = $plugin_data['Name'];

		if ( ! isset( $this->config['version'] ) ) {
			$this->config['version'] = $plugin_data['Version'];
			error_log( var_export($this->config['version'], true) );
		}
...
error_log( var_export($version, true));
		...
		if ( $this->overrule_transients() || ( !isset( $version ) || !$version || '' == $version ) ) {
			...
			$raw_response = $this->remote_get( 'https://api.github.com/repos/epfl-idevelop/wpforms-epfl-payonline/releases/latest' );
			...
			if (is_array($raw_response)) {
				if (!empty($raw_response['body'])) {
					$api_data = json_decode($raw_response['body']);
					$version = ltrim($api_data->tag_name, 'v');
				}
			}
	...
	// 		error_log( var_export($raw_response, true) );
	// 
	// 		if ( is_wp_error( $raw_response ) )
	// 			$version = false;
	// 
	...
	public function get_plugin_data() {
		...
		// error_log($this->config['slug']);
		$data = get_plugin_data( WP_PLUGIN_DIR.'/wpforms-epfl-payonline/'.$this->config['slug'] );
